...
SEARCH_SPACE = {
    'empty': [20, 35, 50],
    'monotonicity': [-0.8, -1.2, -1.8],
    'smoothness': [-1.0, -1.5, -2.0],
    'corner_max': [8, 12, 16]
}

def evaluate_params(empty_val, mono_val, smooth_val, corner_val):
    WEIGHTS_V3['empty'] = empty_val
    WEIGHTS_V3['monotonicity'] = mono_val
    WEIGHTS_V3['smoothness'] = smooth_val
    WEIGHTS_V3['corner_max'] = corner_val
    scores = []
...
def main():
    ...
    with open(LOG_FILE, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['empty', 'monotonicity', 'smoothness', 'corner_max', 'avg_score'])
            ...
            for mono_val in SEARCH_SPACE['monotonicity']:
                for smooth_val in SEARCH_SPACE['smoothness']:
                    for corner_val in SEARCH_SPACE['corner_max']:
                        avg = evaluate_params(empty_val, mono_val, smooth_val, corner_val)
                        writer.writerow([empty_val, mono_val, smooth_val, corner_val, f"{avg:.2f}"])
                        print(f"empty={empty_val}, mono={mono_val}, smooth={smooth_val}, "
                              f"corner={corner_val} => avg={avg:.2f}")
                        if avg > best_score:
                            best_score = avg
